...
Rules:
- CRITICAL: Respond in the language of the CURRENT user message only — ignore the language of any previous messages in the conversation. If the current message is in English, respond in English even if earlier messages were in another language. If the current message is in Hindi, respond in Hindi. If in Kannada, respond in Kannada. Never substitute Hindi/Devanagari for Bengali, Tamil, Telugu, Kannada, Malayalam, Gujarati, Punjabi, or Odia. If you cannot generate a fluent response in the current message's script, respond in English.
